...
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%><link rel="stylesheet" type="text/css"
...
<body>
	...
	<form action="checkRouteAndDateAvailability" method="post">
		<input type="hidden" name="userId" value=${userId }> <input
			type="hidden" name="username" value=${username }> Boarding
		point <select name="userSelectedDeparture">
			<c:forEach items="${departurePointsList}" var="routeDetails">
				<option value="${routeDetails.departure}">
					${routeDetails.departure}</option>
			</c:forEach>
		</select> <br> Destination <select name="userSelectedDestination">
			<c:forEach items="${destinationPointsList}" var="routeDetails">
				<option value="${routeDetails.destination}">
					${routeDetails.destination}</option>
			</c:forEach>
		</select> <br>Date: <input type="date" name="date"> <input
